...
/**
 *  An advanced, compact (one-file only), lightweight, object-oriented MySQL database wrapper built upon PHP's MySQL
 *  extension. It provides methods for interacting with MySQL databases that are more intuitive and fun to use than
 *  PHP's default ones.
 *
 *  The class provides a comprehensive debugging interface with detailed information about the executed queries:
 *  execution time, returned/affected rows, excerpts of the found rows, error messages, etc. It also automatically
 *  EXPLAIN's each SELECT query (so you don't miss those keys again!).
 *
 *  It encourages developers to write maintainable code and provides a better default security layer by automatically
 *  escaping strings.
 *
 ...
 *  @version    2.0 (last revision: January 24, 2011)
...
class Zebra_Database
{
     ...
     *  that many rows, can cause your script to crash due to memory limitations. In this case you should either lower the
     *  value of this property or try and set PHP's memory limit higher using:</i>
     *
     *  <code>
     ...
     *
     *  <code>
     *  // show the debug console only to specifie IPs
    ...
    /**
     *  Closes the MySQL connection associated with the specified link identifier.
     *
     *  @param  link_identifier     $link_identifier    Link identified as returned by the {@link connect()} method.
     *
     *                                                  If not specified, the last opened link will be used.
     *
     *  @return boolean                                 Returns TRUE on success or FALSE on failure.
     */
    function close($link_identifier = '')
    {

        // if no link identifier specified
        if ($link_identifier == '') {

            // close the last one open
            @mysql_close($this->link_identifier);

        // if link identifier was specified
        } else {

            // close the specific link
     ...
     *                              arguments, no new link will be established, but instead, the link identifier of the
     ...
     *                              Default is FALSE.
     *
     *  @return mixed               Returns a MySQL link identifier on success, or FALSE on error.
     */
    function connect($host, $user, $password, $database, $is_new = false)
    {

        // tries to connect to the MySQL database
        // and if connection could not be established
        if (!($this->link_identifier = @mysql_pconnect($host, $user, $password, $is_new))) {
        ...
        // if connection could be established
            ...
            // select the database
            // and if database could not be selected
            if (!($this->database = @mysql_select_db($database, $this->link_identifier))) {
        ...
        // if script gets this far, return the link identifier
     ...
     *
     *                                  Default is "" (an emty string).
     *
     *  @param  array   $replacements   (Optional) An array with as many items as the total "?" symbols in <i>$column</i>,
     *                                  <i>$table</i> and <i>$where</i>. Each item will be automatically {@link escape()}-ed
     *                                  and will replace the corresponding "?".
     *
     *                                  Default is "" (an emty string).
     *
     ...
     *                                  Default is "" (an emty string).
     *
     *  @param  array   $replacements   (Optional) An array with as many items as the total "?" symbols in <i>$table</i>
     *                                  and <i>$where</i>. Each item will be automatically {@link escape()}-ed and will
    ...
    /**
     *  Returns one or more values from ONE row of a table.
     *
     *  <code>
     ...
     *
     *  <i>Works even if {@link http://www.php.net/manual/en/info.configuration.php#ini.magic-quotes-gpc magic_quotes} is ON.</i>
     *
     *  <code>
     ...
     *
     *  @return mixed                   Returns an associative array that corresponds to the fetched row, or FALSE if
     ...
     *                                  by the previous query or from the resource given as argument. Returns FALSE on
     *                                  error.
     */
    ...
    /**
     *  Returns an object with properties that corresponds to the fetched row and moves the internal data pointer ahead.
     ...
     *                                  <i>If not specified, the resource returned by the last run query is used.</i>
     *
     *  @return mixed                   Returns an object with properties that corresponds to the fetched row, or FALSE
    ...
    /**
     *  Returns an associative array containing all the rows (as objects) from the resource created by the previous query or from the
     *  resource given as argument).
     *
     *  <code>
     *  // run a query
     ...
     *                                  created by the previous query or from the resource given as argument. Returns
     *                                  FALSE on error.
     */
    ...
    /**
     *  Returns information about the columns of a given table, as an associtative array.
     *
     *  <code>
     ...
     *
     *  @return array           Returns information about the columns of a given table, as an associtative array.
     */
     ...
     */
    function insert($table, $columns, $highlight = false)
    {
     ...
     *
     *  <i>If you must parse a very large file and your script crashed due to time-out or because of memory limitations,
     *  try the following:</i>
     *
     *  <code>
     ...
     *  resouce arguments, work on the LAST returned result resource!</b>
     *
     *  <code>
     *  // run a query
     *  $db->query('
     ...
     *
     *  @param  array   $replacements   (Optional) An array with as many items as the total "?" symbols in $sql. Each
     *                                  item will be automatically {@link escape()}-ed, enclosed in quotes, and will
     *                                  replace the corresponding "?"
     *
     *                                  Default is "" (an emty string).
     *
     ...
     *                                  returned if there was no LIMIT applied to the query.
     *
     *                                  This is very useful for creating paginations or computing averages. Also, note
     ...
     *                                  or FALSE on error.
     *
     *                                  <i>If query results are taken from cache, the returned result will be a pointer
        ...
        if ($this->_connected()) {
            ...
            if ($this->last_result !== false) {
                ...
                if (is_resource($this->last_result) || $this->last_result === true) {
                    ...
                    if ($is_select && $cache !== false) {
                        ...
                        if (isset($file_name)) {
                            ...
                            // save also the found_rows and returned_rows properties
                ...
                // if query was read from cache
                    ...
                    // the last entry in the cache file contains the returned_rows and found_rows properties
                    // we need to take them off the array
                    $counts = array_pop($this->cached_results[$this->last_result]);

                    // set these two properties from the values in the cached file
     ...
     *  @return boolean                 Returns TRUE on success of FALSE on error
     */
    function select($columns, $table, $where = '', $replacements = '', $cache = false, $highlight = false, $calc_rows = false)
    {
            ...
            foreach (array_keys($blocks) as $block) {
                ...
                if (isset($this->debug_info[$block])) {
                    ...
                    foreach ($this->debug_info[$block] as $debug_info) {
                        ...
                        $output .= '
                            <table cellspacing="0" cellpadding="0" class="zdc-entry' .
                        ...
                        if (isset($debug_info['message']) && trim($debug_info['message']) != '') {
                            ...
                            $output .= '
                                <div class="zdc-box zdc-error">
                                    <strong>' . $this->language['error'] . '</strong>: ' . $debug_info['message'] . '
                                </div>
                            ';
                        }
                        ...
                        if (isset($debug_info['error']) && trim($debug_info['error']) != '') {
                            ...
                            $output .= '
                                <div class="zdc-box zdc-error">
                                    <strong>' . $this->language['mysql_error'] . '</strong>: ' . $debug_info['error'] . '
                                </div>
                            ';
                        ...
                        // common actions (backtrace information, to top, close all)
                        $output .= '
                    ...
                    // globals to show
                    $globals = array('POST', 'GET', 'COOKIE', 'FILES', 'SERVER');
                    ...
                    foreach ($globals as $global) {
                        ...
                        $output .= '
                            <table cellspacing="0" cellpadding="0" id="zdc-globals-' . strtolower($global) . '" class="zdc-entry">
                                <tr>
